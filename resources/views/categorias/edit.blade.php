...
<div class="container cont">
    ...
    <div>
        <center>
            <div class=""><br>
                <form action="{{ route('categorias.update', $categorias) }}" method="POST" class="form shadow p-3 mb-5 rounded ">
                    @csrf
                    @method('PUT')
                    <h1>FORMULARIO DE CATEGORIAS</h1>
                    <div><br><br>
                        <label for=" titulo" class="form-label">Descripcion</label>
                        <input type="text" class="form-control" id="titulo" name="titulo" value="{{ old('titulo', $categorias->titulo) }}">
                        @error('titulo')
